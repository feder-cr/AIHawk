...
This wiki is maintained by the people who build [AIHawk](https://github.com/feder-cr/invisible_playwright_mcp); the goal is a fair split of where each approach wins, not a pitch for one side.
...
- [feder-cr/invisible_playwright_mcp](https://github.com/feder-cr/invisible_playwright_mcp), plus its source in this repository (`src/aihawk/cli.py`, `src/aihawk/web.py`), retrieved 2026-09-05, for the claim that AIHawk ships no built-in approval console.
...
*Written while maintaining [AIHawk](https://github.com/feder-cr/invisible_playwright_mcp), the agent named above, not the RPA platforms; that is why the governance section concedes a gap in our own category too.*
